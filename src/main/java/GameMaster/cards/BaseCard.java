...
import static GameMaster.util.GeneralUtils.removePrefix;
import static GameMaster.util.TextureLoader.getCardTextureString;

public abstract class BaseCard extends CustomCard {
    // ---------- packed ID helper (so each card only changes one line) ----------
    /** Converts class package + simple name into "GameMaster:Pack:Name". */
    public static String packedID(Class<?> cls) {
        return BasicMod.makeID(packFromClass(cls) + ":" + cls.getSimpleName());
    }
    private static String packFromClass(Class<?> cls) {
        String pkg = cls.getPackage().getName();   // e.g. GameMaster.cards.General
        int dot = pkg.lastIndexOf('.');
        String leaf = (dot >= 0) ? pkg.substring(dot + 1) : pkg; // "General"
        if (leaf.isEmpty()) return "General";
    ...
    private static String getName(String ID) { return CardCrawlGame.languagePack.getCardStrings(ID).NAME; }
    private static String getInitialDescription(String ID) { return CardCrawlGame.languagePack.getCardStrings(ID).DESCRIPTION; }

    // ---------- Constructor helpers ----------
    protected final void setDamage(int damage) { this.setDamage(damage, 0); }
    protected final void setDamage(int damage, int damageUpgrade) {
        this.baseDamage = this.damage = damage;
        if (damageUpgrade != 0) { this.upgradeDamage = true; this.damageUpgrade = damageUpgrade; }
    }

    protected final void setBlock(int block) { this.setBlock(block, 0); }
    protected final void setBlock(int block, int blockUpgrade) {
        this.baseBlock = this.block = block;
        if (blockUpgrade != 0) { this.upgradeBlock = true; this.blockUpgrade = blockUpgrade; }
    }

    protected final void setMagic(int magic) { this.setMagic(magic, 0); }
    protected final void setMagic(int magic, int magicUpgrade) {
        this.baseMagicNumber = this.magicNumber = magic;
        if (magicUpgrade != 0) { this.upgradeMagic = true; this.magicUpgrade = magicUpgrade; }
    }

    protected final void setCustomVar(String key, int base) { this.setCustomVar(key, base, 0); }
    ...
    protected enum VariableType { DAMAGE, BLOCK, MAGIC }
    protected final void setCustomVar(String key, VariableType type, int base) { setCustomVar(key, type, base, 0); }
    protected final void setCustomVar(String key, VariableType type, int base, int upgrade) {
        setCustomVarValue(key, base, upgrade);
        switch (type) {
            case DAMAGE: calculateVarAsDamage(key); break;
            case BLOCK:  calculateVarAsBlock(key);  break;
        }
        if (!customVars.containsKey(key)) {
            QuickDynamicVariable var = new QuickDynamicVariable(key);
            customVars.put(key, var);
        ...
        switch (type) {
            case DAMAGE:
                setVarCalculation(key, (c, m, baseVal)->{
                    boolean wasMultiDamage = c.isMultiDamage; c.isMultiDamage = false;
                    int origBase = c.baseDamage, origVal = c.damage;
                    c.baseDamage = preCalc.apply(c, m, baseVal);
                    if (m != null) c.calculateCardDamage(m); else c.applyPowers();
                    c.damage = postCalc.apply(c, m, c.damage);
                    c.baseDamage = origBase; c.isMultiDamage = wasMultiDamage;
                    int result = c.damage; c.damage = origVal; return result;
                });
                break;
            case BLOCK:
                setVarCalculation(key, (c, m, baseVal)->{
                    ...
                    int result = c.block; c.block = origVal; return result;
                });
                break;
            default:
                setVarCalculation(key, (c, m, baseVal)->{
    ...
    private void setCustomVarValue(String key, int base, int upg) {
        cardVariables.compute(key, (k, old)->{
            ...
            old.base = base; old.upgrade = upg; return old;
        });
    }
    ...
    protected final void colorCustomVar(String key, Color normalColor, Color increasedColor, Color decreasedColor, Color upgradedColor) {
        ...
        var.normalColor = normalColor; var.increasedColor = increasedColor; var.decreasedColor = decreasedColor; var.upgradedColor = upgradedColor;
    }
    ...
    protected void calculateVarAsDamage(String key) {
        setVarCalculation(key, (c, m, base)->{
            boolean wasMultiDamage = c.isMultiDamage; c.isMultiDamage = false;
            int origBase = c.baseDamage, origVal = c.damage;
            c.baseDamage = base; if (m != null) c.calculateCardDamage(m); else c.applyPowers();
            c.baseDamage = origBase; c.isMultiDamage = wasMultiDamage;
            int result = c.damage; c.damage = origVal; return result;
        });
    }
    protected void calculateVarAsBlock(String key) {
        setVarCalculation(key, (c, m, base)->{
            int origBase = c.baseBlock, origVal = c.block;
            c.baseBlock = base; if (m != null) c.calculateCardDamage(m); else c.applyPowers();
            c.baseBlock = origBase; int result = c.block; c.block = origVal; return result;
        });
    }
    ...
    public int customVarBase(String key) {
        ...
        return var == null ? -1 : var.base;
    }
    public int customVar(String key) {
        LocalVarInfo var = cardVariables == null ? null : cardVariables.get(key);
        return var == null ? -1 : var.value;
    }
    public int[] customVarMulti(String key) {
        LocalVarInfo var = cardVariables.get(key);
        return var == null ? null : var.aoeValue;
    }
    public boolean isCustomVarModified(String key) {
        LocalVarInfo var = cardVariables.get(key);
        return var != null && var.isModified();
    }
    public boolean customVarUpgraded(String key) {
        LocalVarInfo var = cardVariables.get(key);
        return var != null && var.upgraded;
    }

    protected final void setCostUpgrade(int costUpgrade) { this.costUpgrade = costUpgrade; this.upgradeCost = true; }
    ...
    protected final void setExhaust(boolean baseExhaust, boolean upgExhaust) { this.baseExhaust = baseExhaust; this.upgExhaust = upgExhaust; this.exhaust = baseExhaust; }
    protected final void setEthereal(boolean baseEthereal, boolean upgEthereal) { this.baseEthereal = baseEthereal; this.upgEthereal = upgEthereal; this.isEthereal = baseEthereal; }
    protected void setInnate(boolean baseInnate, boolean upgInnate) { this.baseInnate = baseInnate; this.upgInnate = upgInnate; this.isInnate = baseInnate; }
    protected void setSelfRetain(boolean baseRetain, boolean upgRetain) { this.baseRetain = baseRetain; this.upgRetain = upgRetain; this.selfRetain = baseRetain; }
    ...
    public AbstractCard makeStatEquivalentCopy() {
        ...
        if (candidate instanceof BaseCard) {
            ...
            for (Map.Entry<String, LocalVarInfo> varEntry : cardVariables.entrySet()) {
                LocalVarInfo target = card.getCustomVar(varEntry.getKey()), current = varEntry.getValue();
                if (target == null) {
    ...
    public void upgrade() {
        if (!upgraded) {
            ...
            for (LocalVarInfo var : cardVariables.values()) upgradeCustomVar(var);
    ...
    protected void upgradeCustomVar(String key) {
        LocalVarInfo var = cardVariables.get(key);
        if (var == null) throw new NullPointerException("Custom variable with key " + key + " does not exist in " + getClass().getName());
    ...
    protected void upgradeCustomVar(LocalVarInfo var) { upgradeCustomVar(var, var.upgrade); }
    protected void upgradeCustomVar(LocalVarInfo var, int amt) {
        if (amt != 0) { var.base += amt; var.value = var.base; var.upgraded = true; }
    }
    ...
    public void applyPowers() {
        if (!inCalc) {
            inCalc = true;
            for (LocalVarInfo var : cardVariables.values()) var.value = var.calculation.apply(this, null, var.base);
            if (isMultiDamage) {
                ...
                for (LocalVarInfo var : cardVariables.values()) {
                    ...
                    for (int i = 0; i < monsters.size(); ++i) { m = monsters.get(i); var.aoeValue[i] = var.calculation.apply(this, m, var.base); }
                }
            }
            inCalc = false;
        }
    ...
    public void calculateCardDamage(AbstractMonster m) {
        if (!inCalc) {
            ...
            if (isMultiDamage) {
                ...
                for (LocalVarInfo var : cardVariables.values()) {
                    if (var.aoeValue == null || var.aoeValue.length != monsters.size()) var.aoeValue = new int[monsters.size()];
                    for (int i = 0; i < monsters.size(); ++i) { var.aoeValue[i] = var.calculation.apply(this, monsters.get(i), var.base); }
                }
            }
            inCalc = false;
        }
    ...
    private static class QuickDynamicVariable extends DynamicVariable {
        ...
        public QuickDynamicVariable(String key) { this.localKey = key; this.key = makeID(key); }
        ...
        @Override public boolean isModified(AbstractCard c) { return c instanceof BaseCard && (current = (BaseCard) c).isCustomVarModified(localKey); }
        @Override public int value(AbstractCard c) { return c instanceof BaseCard ? ((BaseCard) c).customVar(localKey) : 0; }
        @Override public int baseValue(AbstractCard c) { return c instanceof BaseCard ? ((BaseCard) c).customVarBase(localKey) : 0; }
        @Override public boolean upgraded(AbstractCard c) { return c instanceof BaseCard && ((BaseCard) c).customVarUpgraded(localKey); }

        public Color getNormalColor() { LocalVarInfo v; if (current == null || (v = current.getCustomVar(localKey)) == null) return Settings.CREAM_COLOR; return v.normalColor; }
        public Color getUpgradedColor() { LocalVarInfo v; if (current == null || (v = current.getCustomVar(localKey)) == null) return Settings.GREEN_TEXT_COLOR; return v.upgradedColor; }
        public Color getIncreasedValueColor() { LocalVarInfo v; if (current == null || (v = current.getCustomVar(localKey)) == null) return Settings.GREEN_TEXT_COLOR; return v.increasedColor; }
        public Color getDecreasedValueColor() { LocalVarInfo v; if (current == null || (v = current.getCustomVar(localKey)) == null) return Settings.RED_TEXT_COLOR; return v.decreasedColor; }
    }
    ...
    protected static class LocalVarInfo {
        ...
        public LocalVarInfo(int base, int upgrade) { this.base = this.value = base; this.upgrade = upgrade; }
        private static int noCalc(BaseCard c, AbstractMonster m, int base) { return base; }
        public boolean isModified() { return forceModified || base != value; }
    }
}
